...
function ensureCleanWorktree() {
  ...
  if (status) {
    throw new Error(
      "Working tree is not clean. Commit or stash changes before drafting a release.",
    );
  }
}
...
function ensureBranch() {
  ...
  if (branch !== "main") {
    throw new Error(
      `Release drafts must be created from main. Current branch: ${branch}`,
    );
  }
}
...
function assertEqual(left, right, description) {
  if (left !== right) {
    throw new Error(
      `Version mismatch between ${description}: ${left} !== ${right}`,
    );
  }
}
